...
class VistaListaEntrenamientos(QWidget):
    ...
    def mostrar_entrenamientos(self, id_persona, lista_entrenamientos):
        ...
        nombre_completo = self.persona['nombre'] + " " + self.persona['apellido']
        ...
        for entrenamiento in self.entrenamientos:
            ...
            etiqueta_nombre=QLabel(entrenamiento['ejercicio'])
            ...
            etiqueta_fecha = QLabel(str(entrenamiento['fechaRealizacionEjercicio']))
            ...
            etiqueta_repeticiones = QLabel(str(entrenamiento['numeroRepeticiones']))
            ...
            etiqueta_tiempo = QLabel(str(entrenamiento['duracion']))
            ...
            boton_editar.clicked.connect(partial(self.mostrar_dialogo_editar_entrenamiento, entrenamiento))
            ...
            boton_eliminar.clicked.connect(partial(self.eliminar_entrenamiento, entrenamiento['id']))
            self.distribuidor_tabla_entrenamientos.addWidget(boton_eliminar, numero_fila + 1, 5, Qt.AlignTop)

            numero_fila=numero_fila+1

        #TODO: Restaurar cuando se haga integración de retirar persona 
    ...
    def mostrar_dialogo_editar_entrenamiento(self, entrenamiento):
        """
        Esta función ejecuta el diálogo para editar un entrenamiento
        """    
        dialogo=VistaCrearEntrenamiento(entrenamiento, self.interfaz, self.ejercicios)
        dialogo.exec_()
        if dialogo.resultado==1:            
            self.interfaz.editar_entrenamiento(entrenamiento['id'], self.persona_actual, dialogo.combobox_ejercicios.currentText(), dialogo.fecha.text(), dialogo.texto_repeticiones.text(),
                                          dialogo.texto_tiempo.text())
            self.hide()

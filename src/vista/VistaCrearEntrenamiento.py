...
class VistaCrearEntrenamiento(QDialog):
    ...
    def __init__(self, entrenamiento, interfaz, ejercicios):
        ...
        if (entrenamiento != None):
            ...
            self.fecha.setDate(QtCore.QDate.fromString(str(entrenamiento["fechaRealizacionEjercicio"]), "yyyy-MM-dd"))
            self.texto_repeticiones.setText(str(entrenamiento["numeroRepeticiones"]))
            self.texto_tiempo.setText(str(entrenamiento["duracion"]))

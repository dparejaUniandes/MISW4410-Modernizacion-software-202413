...
class VistaListaEjercicios(QWidget):
    ...
    def mostrar_ejercicios(self, lista_ejercicios):
        ...
        for ejercicio in self.ejercicios:
            ...
            boton_editar.clicked.connect(partial(self.mostrar_dialogo_editar_ejercicio, ejercicio))
            ...
            etiqueta_eliminar.clicked.connect(partial(self.eliminar_ejercicio, ejercicio.id))
    ...
    def mostrar_dialogo_crear_ejercicio(self):
        ...
        if dialogo.resultado==1:
            calorias = 0
            caloriasTexto = dialogo.texto_calorias.text()
            if(len(caloriasTexto) > 0 and caloriasTexto.isnumeric()):
                calorias = int(caloriasTexto)
            self.interfaz.crear_ejercicio(dialogo.texto_nombre.text(), dialogo.texto_descripcion.toPlainText(), dialogo.texto_enlace.text(), calorias)

    def mostrar_dialogo_editar_ejercicio(self, ejercicio):
        """
        Esta función ejecuta el diálogo para editar un ejercicio
        """    
        dialogo=VistaCrearEjercicio(ejercicio, self.interfaz)
        dialogo.exec_()
        if dialogo.resultado==1:
            calorias = 0
            caloriasTexto = dialogo.texto_calorias.text()
            if(len(caloriasTexto) > 0 and caloriasTexto.isnumeric()):
                calorias = int(caloriasTexto)
            self.interfaz.editar_ejercicio(
                ejercicio.id,
                dialogo.texto_nombre.text(),
                dialogo.texto_descripcion.toPlainText(),
                dialogo.texto_enlace.text(),
                calorias
            )
            self.hide()
    ...
    def eliminar_ejercicio(self, id_ejercicio):
        ...
        if respuesta == QMessageBox.Yes:
            self.interfaz.eliminar_ejercicio(id_ejercicio)
            self.hide()
            self.interfaz.mostrar_ejercicios()
